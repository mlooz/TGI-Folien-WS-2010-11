...
\end{quote}
\item Gib eine Grammatik an, die die Sprache $L_1 = \{a^ib^jc^k \mid  i = j\mbox{ oder }j = k$ erzeugt
...
\begin{enumerate}
\item $\{a^n\#a^2n\#a^3n\}$
\item $\{w\#t | w\mbox{ ist Teilstring von }t \mbox{ und } w,t \in \{0,1\}^*\}$
\item $\{w | w \in \{a,b\} \mbox{und $w$ enthält doppelt soviele $a$ wie $b$ }\}$
\item Das Komplement von $\{a^nb^n | n \geq 0\}$
\item $\{a\#b | a,b \in \{0,1\}^* \mbox{ und } x \neq y\}$
\end{enumerate}
}{
\begin{enumerate}
