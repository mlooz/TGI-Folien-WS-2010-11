...
\item Trotz ähnlicher Bennenung hat eine Nichtdeterministische Turingmaschine nichts mit Orakelturingmaschienen zu tun!
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Kurzer Exkurs}
Einer der Hauptgründe, warum die $P \stackrel{?}{=} NP$ Frage so schwer ist, is das bekannt ist, das es zwei Sprachen $A$ und $B$ gibt, für die gilt:
\begin{enumerate}
\item $\mathcal{P}^A = \mathcal{NP}^A$
\item $\mathcal{P}^B \neq \mathcal{NP}^B$
\end{enumerate}
Ein Beweiß für eine der beiden Aussagen darf also nicht mit Orakel weiterhin funktionieren.
\end{frame}
