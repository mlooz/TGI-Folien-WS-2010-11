...
\end{block}
Um zu zeigen, dass $L_u$ unentscheidbar ist, wollen wir wie folgt vorgehen: Wie nehmen an wir könnten $L_u$ entscheiden und zeigen, dass wir unter dieser Vorraussetzung auch $L_d^c$ entscheiden können.
\begin{itemize}
...
\begin{enumerate}
\item Es existiert eine polynomielle deterministische Turing-Maschine, die $f$ berechnet
...
% NP vollständigkeit
\begin{frame}
\frametitle{$\mathcal{NP}$-Härte}
\begin{block}{$\mathcal{NP}$-Härte}
Eine Sprache $L_1$ ist $\mathcal{NP}$-Hart (oder schwer)  gdw. 
\[\forall L_2 \in \mathcal{NP} \,\, : \,\, L_2 \propto L_1\]
\end{block}
\textbf{Anmerkung}: In diesem Sinne sind die $\mathcal{NP}$-harten Probleme schwerer oder mindestens so schwer zu lösen wie alle $\mathcal{NP}$ Sprachen.
\end{frame}

% Polyreduktion ist Trasitiv, daher ist NPC cool
\begin{frame}
\frametitle{Transitivität von Polyreduktionen und $\mathcal{NP}$}
Polynomiale Transformationen sind offensichtlich transitiv, d.h. wenn $L_1 \propto L_2$ und $L_2 \propto L_3$ dann gilt auch $L_1 \propto L_3$.\\[8pt]
...
Gegeben eine Sprache $L_N$, von der wir wissen das sie $\mathcal{NP}$-Schwer ist, was wäre ein möglicher Ansatz um für eine weitere Sprache $L_X$ die $\mathcal{NP}$-Härte zu beweisen?\\\pause
...
$$L \in \mathcal{NP}$$ sowie $$L\mbox{ ist $\mathcal{NP}$-Hart}$$
Damit sind die $\mathcal{NP}$-Vollständigen Probleme, die ,,schwersten'' Probleme aus $\mathcal{NP}$.\\
Interesannt ist $\mathcal{NP}$ vorallem da man aus Aussagen über diese Probleme viel über alle Probleme aus $\mathcal{NP}$ aussagen kann. Ist etwa $SAT \in \mathcal{P}$ so wäre $\mathcal{P} = \mathcal{NP}$, (warum?)
\end{frame}
...
\end{block}
\begin{block}{Errinerung:}
Eine Clique ist ein vollständig verbundener Teilgraph, also eine Menge $V' \subset V$, so dass für alle $i,j \in V'$ mit $i\neq j$ gilt:$(i, j) \in E$.
\end{block}
\textit{Dieses Problem ist $\mathcal{NP}$-Vollständg}
\end{frame}
...
$CLIQUE \in \mathcal{NP}$: Übung\\
Wir Zeigen $3SAT \propto CLIQUE$. (warum?)\\\pause
...
$$E = {(v_{i,j},v_{k,l})| (i \neq k \wedge x_{i,j} \neq \bar{x_{k,l}} )}$$%TODO Bar sucks, \lineover or smth?
$$K = n$$
\end{frame}
...
In der Regel ist es schwer zu zeigen, dass ein Problem $\mathcal{NP}$-schwer/hart ist, aber leicht zu zeigen, dass ein Problem in $\mathcal{NP}$ liegt.\\[8pt]
Da man üblicherweise ein $\mathcal{NP}$-Vollständiges Problem als Vorrausetzung für die Reduktion benötigt (Ausnahme: Satz von Cook), lohnt es sich einige kennen zu lernen.
\end{frame}

\begin{frame}
\frametitle{SAT}
\begin{block}{Problem}
\textbf{Gegeben:} Menge $U$ von Variablen\\
...
\begin{block}{Problem}
\textbf{Gegeben:} Menge $U$ von Variablen\\
Menge $C$ von Klauseln über $U$, jede Klausel enthält genau \emph{drei} Literale\\
\textbf{Frage:} Existiert eine erfüllende Wahrheitsbelegung von $C$?
\end{block}
\begin{itemize}
...
\end{block}
\begin{itemize}
\item Bei festem $K$ nur $\mathcal{NPC}$ für $K \geq 3$ (sonst in $\mathcal{P}$).
\end{itemize}
\end{frame}
\begin{frame}
...
\textbf{Frage:} Gibt es ein Teilmenge $\mathcal{S}^*$ von $\mathcal{S}$, so dass jedes Element aus $X$ in genau einem der $s \in \mathcal{S}^*$ enthalten ist
\end{block}
\end{frame}
...
Sei $S = \{s_{11}, s_{12},\ldots,s_{19},s_{21},\ldots,s_{99}\}$ die Menge der Sudoku Kästchen.
...
Die Menge aller $S_{ijkl}$ ist unsere Menge an Teilmengen.% jedes der Paare garantiert eine der Bedingungen für Sudoku: Es darf etwa nur eine Zahl in einem Kästchen stehen und jede Zahl darf in jeder Reihe, Spalte und in jedem Block nur einmal erscheinen. 
\end{frame}

\begin{frame}
\frametitle{SUBSET-SUM}
\begin{block}{Problem}
\textbf{Gegeben:} Eine Menge $S$ von Ganzzahlen\\
Eine Ganzzahl $n$.
\textbf{Frage:} Gibt es eine Teilmenge $S^*$, so dass die Summe der Elemente der Teilmenge $n$ ist?
\end{block}
\end{frame}
%% - PARTITION
\begin{frame}
...
Es gibt (sehr) viele Probleme von denen man weiß, dass sie $NP$-Vollständig sind, es gibt ein "Klassisches" Paper von Richard Karp in dem 21 Probleme vorgestelt wurden die großteils recht bekannt sind.\\
Wer nicht genug bekommen kann, darf in "Introduction to the Theory of Computation" von "Michael Sipser" schauen, dort gibt es ~200.\\[10pt]
...
TODO:
 - Alles nochmal sorgfältig durchlesen/überarbeiten
 - Sudoku als Exact Cover Beispiel formulieren
 - bar fixen bein CLIQUE
