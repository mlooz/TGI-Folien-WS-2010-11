...
\end{block}
Um zu zeigen, dass $L_u$ unentscheidbar ist, wollen wir zeigen, dass wir, wenn wir $L_u$ entscheiden könnten auch $L_d^c$entscheiden können.
\end{frame}

\begin{frame}
\frametitle{Polyreduktionen}
\begin{block}{Definition (Vorlesung)}
Eine polynomiale Transformation einer Sprache $L_1$ in eine Sprache $L_2$ ist eine Funktion
...
Ist $L_3 \in \mathcal{NP}$, so wissen wir, dass auch $L_1$ sowie $L_2 \in \mathcal{NP}$, man sagt wir haben $L_1$ und $L_2$ auf $L_3$ polynomiell reduziert.\\[8pt]
