...
Es gibt mehrere, zu Turingmaschienen (bzgl. Berechenbarkeit) äquivalente Berechnungsmodelle, die der Turingmaschine sehr ähnlich sind. Man spricht hier von Erweiterungen von Turingmaschinen. Diese verwendet man gerne in z.B. Beweisen, da sie oft Übersichtlicher sind.
\end{block}
\end{frame}
% Mehrkopf
\begin{frame}
\frametitle{Mehrkopf Turingmaschienen}
\vspace{-2cm}
\begin{figure}[H]
\begin{center}
...
\end{center}
\end{figure}
Eine Mehrkopf Turingmaschiene hat mehrere Lese-/Schreibeköpfe. Die Zustandsänderung hängt nun von dem gelesenen Zeichen aller Köpfe ab und kann auch alle Köpfe verschieben bzw. mit allen gleichzeitig schreiben.
...
% Mehrband
\begin{frame}
\frametitle{Mehrband Turingmaschienen}
\vspace{-2cm}
\begin{figure}[H]
\begin{center}
\begin{tikzpicture}[scale=0.3]
...
\end{center}
\end{figure}
Eine Mehrband Turingmaschiene hat mehrere Bänder. Die Zustandsänderung kann nun auch auf ein anderes Band wechseln. Dabei wird immer auf die zuletzt eingenommene Position auf dem anderen Band gewechselt 
\begin{block}{Änderungen in der Definition}
...
Der Satz von Rice sagt aus, dass es unmöglich ist eine nichttrivial eigenschaft von Turingmaschinen algorithmisch zu entscheiden.
...
\begin{itemize}
\item Alle Turingmaschienen die nur $0$ aufs Band Schreiben.
\item Alle Turingmaschienen die genau eine $0$ aufs Band Schreiben.
\item Alle Turingmaschienen die überhaupt etwas auf das Band schreiben.
\end{itemize}
\end{frame}

% Post'sches Korrespondenzproblem
% Aufgaben
% Beispiel mit Schwerem PCP
