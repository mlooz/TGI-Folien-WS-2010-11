...
\(\Sigma := \{a,b\}\)
\begin{itemize}
 \item \(L_1 :=\) Alle Worte, die nur aus einer graden Anzahl as bestehen.
\item \(L_2 :=\) Alle Worte, die gleich viele as und bs enthalten.
\item \(L_1\) ist regulär, \(L_2\) nicht.
\end{itemize}
\end{frame}

\begin{frame}
 \frametitle{Wiederholung der Wiederholung: Reguläre Ausdrücke}
	%TODO
\end{frame}
...
\begin{frame}
	...
	\begin{enumerate}
	\item Tafel (kommt noch in online Folien)
	\end{enumerate}
\end{frame}
\begin{frame}
...
\begin{frame}
	...
	\begin{enumerate}
	\setcounter{enumi}{1}
	\item $$L = a(ba)^*(b\cup\varepsilon) \cup b(ab)^*(a\cup\varepsilon)$$
	\begin{center}\includegraphics[scale=0.7]{pics/dea1.pdf}\end{center}
	\end{enumerate}
\end{frame}
\begin{frame}
...
\begin{frame}
	...
	\KITframe[another gray background. Gray is really awesome!] 
	{\parbox{\textwidth}{
	    \(q_1, q_2 \in Q\), \(a \in \Sigma\)
	}}\\[0.2cm]	
...
\begin{frame}
	\frametitle{NEA2DEA: Lösung}
	\begin{enumerate}
	\item Ein NEA der die Sprache des Regulären Ausdrucks ${(a \cup (ab (b)^* ba))^*}$ akzeptiert ist:\\
  
	\begin{center}
	\begin{figure}
	\begin{tikzpicture}[node distance=2cm]	
	\node[circle,draw,double] (q_0) {$q_0$};
	\node[circle,draw,right of=q_0] (q_1) {$q_1$};
	\node[circle,draw,right of=q_1] (q_2) {$q_2$};
	\draw[->] (-1,0) -- (q_0);
	\draw (q_0) edge [loop above] node {$a$} (q_0);
	\draw[->] (q_0) edge [bend left] node [above] {$a$} (q_1);	
	\draw[->] (q_1) edge [bend left] node [below] {$a$} (q_0);	
	\draw[->] (q_1) edge [bend left] node [above] {$b$} (q_2);	
	\draw[->] (q_2) edge [bend left] node [below] {$b$} (q_1);	
	\draw (q_2) edge [loop above] node {$b$} (q_2);
	\end{tikzpicture}	
	\end{figure}
	\end{center}
	\end{enumerate}
...
\begin{frame}
	...
	\begin{enumerate}
	\setcounter{enumi}{1}
	\item Wende Potenzmengenkonstruktion an:\\
	\begin{center}
	$\begin{array}{r|cc}
	\,& a & b \\\hline
	q_0 & \{q_0, q_1\} & \emptyset\\ \pause
	\{q_0, q_1\} &  \{q_0, q_1\}& \{q_3\} \\ \pause
	q_3	&	\emptyset   & \{q_2,q_3\} \\ \pause
	\{q_2,q_3\}	&	q_1	&	\{q_2,q_3\}\\ \pause
	\emptyset & \emptyset & \emptyset \\
	\end{array}$
	\end{center}
	\end{enumerate}
